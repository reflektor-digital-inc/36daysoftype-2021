/* eslint-disable */
import { WebGLCubeRenderTarget, Vector2, MeshBasicMaterial, DoubleSide, RGBFormat, LinearFilter, CubeReflectionMapping, EquirectangularReflectionMapping, WebGLRenderTarget, Camera, Scene, Mesh, PlaneBufferGeometry, ShaderMaterial, CubeRefractionMapping, BackSide, NoBlending, BoxBufferGeometry, CubeCamera, Color, LinearMipmapLinearFilter } from 'three';
...
class CustomWebGLCubeRenderTarget extends WebGLCubeRenderTarget {
  ...
  setup(renderer, texture) {
    ...
    const camera = new CubeCamera(1, 100000, this);
        ...
        #include <common>
          ...
          void main (void) {
            ...
            gl_FragColor = vec4(vec3(
              0.5 * pattern(uv * 5.0123 + -0.37 * cos(time * 0.05)),
              0.5 * pattern(uv * 5.0123 +  0.0 * cos(time * 0.05)),
              0.5 * pattern(uv * 5.0123 +  0.37 * cos(time * 0.05))
            ) * diffuse, 1.0);
