...
void main () {
    vec4 wireframe = getStyledWireframe(vCenter);
    if (wireframe.a < 0.75) discard;

    vec2 screenspaceUv = gl_FragCoord.xy/resolution;
    vec3 gradient = texture2D(gradientTexture, screenspaceUv).rgb;
    ...
    gl_FragColor = vec4(finalColor, wireframe.a);
}
